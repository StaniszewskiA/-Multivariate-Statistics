...
````{r Part 2}
fit_poly <- lm(Installed_Capacity_MW ~ poly(Energy_Production_MWh, 4), data = energy_data)
summary(fit_poly)
````

```{r}
fit_poly_raw <- lm(Installed_Capacity_MW ~ poly(Energy_Production_MWh, 4, raw = TRUE), data = energy_data)
...
plot(energy_data$Energy_Production_MWh, energy_data$Type_of_Renewable_Energy, col = "darkgrey", cex = 0.5, xlim = production_lims)
...
fit_bs_knots <- lm(Type_of_Renewable_Energy ~ bs(Energy_Production_MWh, knots = c(25, 40, 60)), data = energy_data)
pred_bs_knots <- predict(fit_bs_knots, list(Energy_Production_MWh = production_grid), se.fit = TRUE)
plot(energy_data$Energy_Production_MWh, energy_data$Type_of_Renewable_Energy, cex = 0.5, col = "darkgrey")
...
```{r}
fit_bs_df <- lm(Type_of_Renewable_Energy ~ bs(Energy_Production_MWh, df = 6), data = energy_data)
...
```{r}
fit_ns <- lm(Type_of_Renewable_Energy ~ ns(Energy_Production_MWh, df = 4), data = energy_data)
...
```{r}
fit_smooth_df <- smooth.spline(energy_data$Type_of_Renewable_Energy, energy_data$Energy_Production_MWh, df = 7)
...
```{r}
fit_smooth_cv <- smooth.spline(energy_data$Type_of_Renewable_Energy, energy_data$Energy_Production_MWh, cv = TRUE)
plot(energy_data$Type_of_Renewable_Energy, energy_data$Energy_Production_MWh, cex = 0.5, col = "darkgrey")
...
for (i in 1:length(spans)) {
   fit_loess <- loess(Type_of_Renewable_Energy ~ Energy_Production_MWh, span = spans[i], data = energy_data)
...
fit_gam_ls <- lm(Type_of_Renewable_Energy ~ ns(Installed_Capacity_MW, df = 4) + ns(Energy_Production_MWh, df = 5) + Energy_Consumption_MWh,
                 data = energy_data)
fit_gam_ls
summary(fit_gam_ls)
```
...
fit_gam_bf <- gam(Type_of_Renewable_Energy ~ s(Installed_Capacity_MW, df = 4) + s(Energy_Production_MWh, df = 5) + Energy_Consumption_MWh, data = energy_data)
summary(fit_gam_bf)
...
```
```{r}
fit_gam_1 <- gam(Type_of_Renewable_Energy ~ s(Installed_Capacity_MW, df = 5) + Energy_Production_MWh, data = energy_data)
fit_gam_2 <- gam(Type_of_Renewable_Energy ~ Energy_Storage_Capacity_MWh + s(Installed_Capacity_MW, df = 5) + Energy_Production_MWh, data = energy_data)
anova(fit_gam_1, fit_gam_2, fit_gam_bf, test = "F")
```
```{r}
fit_gam_lo <- gam(Type_of_Renewable_Energy ~ s(Installed_Capacity_MW, df = 4) + lo(Energy_Production_MWh, span = 0.7) + Energy_Consumption_MWh, 
                  data = energy_data)
summary(fit_gam_lo)
par(mfrow = c(1, 3))
...
```
```{r}
fit_logistic_gam <- gam(Installed_Capacity_MW  + s(Energy_Production_MWh, df = 5) + Energy_Consumption_MWh, 
